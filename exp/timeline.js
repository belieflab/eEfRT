...
let instructions_1 = {
    ...
    stimulus:
        ...
        '<p style="color:white;">You will play the game for exactly <span><strong>' +
        MinutesToPlay +
        " minutes</strong></span>, after which you will be asked to stop.  </p>" +
...
let instructions_2 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">For <b>each trial</b>, you will have an opportunity to <b>win some ' +
        version +
        "</b>.</p> " +
        '<p style="color:white;">In order to be eligible to <b>win ' +
        version +
        "</b> on a given trial, you must first complete a task that requires you to <b>press a button on your keyboard</b> repeatedly for a certain amount of time.</p>" +
...
let instructions_3 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">If you choose the <b>Easy task</b>, you will need to press the <strong >' +
        EasyKey_uCase +
        " key</strong> with your <strong>" +
        handedness.toUpperCase() +
        " index finger</strong> approximately <b>30 times within 7 seconds</b>. </p> " +
        '<p style="color:white;">You are eligible to win <b>' +
        currency +
        practiceEasy[1] +
        " " +
        points +
        "</b> for each time you complete the <b>Easy task</b>. </p> " +
        '<p style="color:white;"><i>Press the spacebar to continue.</i></p>',
    choices: [32],
};
...
let instructions_4 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">If you choose the <b>Hard task</b>, you will need to press the <strong>' +
        HardKey_uCase +
        " key</strong> with the <b>pinky finger</b> of your <strong>" +
        antihandedness.toUpperCase() +
        "</strong> hand approximately <b>100 times within 21 seconds</b>.  </p> " +
        ...
        '<p style="color:white;"><b>Hard trials</b> range from <b>' +
        rangeLow +
        " to around " +
        rangeHigh +
        "</b>.</p>" +
...
let instructions_5 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;"><b>Completing the Easy or Hard task makes you eligible to receive ' +
        version +
        " on that trial</i></b>,<p>" +
        '<p style="color:white;"><b>but completion alone does NOT guarantee that you will win ' +
        version +
        ".</b></p> " +
...
let instructions_6 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;"><b>Some trials are less likely to give you ' +
        version +
        " than others.</b></p> " +
        '<p style="color:white;">Some trials have an <b>88% chance</b> of receiving ' +
        version +
        " if you <b>complete the task</b>.  </p> " +
        '<p style="color:white;">Some trials have a <b>50% chance</b> of receiving ' +
        version +
        " if you <b>complete the task</b>.   </p> " +
        '<p style="color:white;">Some trials have a <b>12% chance</b> of receiving ' +
        version +
        " if you <b>complete the task</b>.   </p> " +
        '<p style="color:white;"><i>Press the spacebar to continue.</i></p>',
    choices: [32],
...
let instructions_7 = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">Also keep in mind that you only have <strong>' +
        MinutesToPlay +
        " minutes</strong> to get through the game, and the Hard task takes about twice as long as the Easy task.   </p> " +
...
let begin_practice = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">Get your hands in position and press the spacebar to start the practice trials. </p> ' +
...
let trial = {
    ...
    on_finish: function (data) {
        ...
        if (selection == EasyKey_uCase.toUpperCase()) {
            // handles exception if participant does not respond within 5s
            ...
            selection =
                selectionTimeout[
                    Math.floor(Math.random() * selectionTimeout.length)
                ]; // randomizes the two possible response keys
...
let load = {
    ...
    stimulus: '<p style="color:white; font-size:24px">Ready?</p>',
    button_html:
        '<button id="ready" onclick="" style="outline:none; background-color:black">START</button>',
    choices: [32],
    // trial_duration: 1000,
...
let buttonPressing = {
    ...
    prompt:
        ...
        feedbackGenerator +
        timeRemaining +
        '<input autocomplete="autocomplete_off_hack_xfr4!k" id="tapTap" type="text" style="background-color:black; outline:none; border:none; background:none" onkeyup="">',
    ...
    on_load: function buttonPress() {
        ...
        barFill.innerHTML =
            "Press the <strong>" +
            selection +
            "</strong> key until the bar fills up.";
                ...
                if (pressing_time == 21000) {
                    if (keycode == HardKey_ASCII) {
                        document
                            ...
                            .setAttribute("onkeyup", "return moveHard()"); // event.charCode allows us to set specific keys to use
                    } else {
                        document
                            .getElementById("counter")
                            .setAttribute("onkeyup", "return false"); // event.charCode allows us to set specific keys to use
                    }
                } else if (pressing_time == 7000) {
                    if (keycode == EasyKey_ASCII) {
                        document
                            .getElementById("counter")
                            .setAttribute("onkeyup", "return moveEasy()"); // event.charCode allows us to set specific keys to use
                    } else {
                        document
                            .getElementById("counter")
                            .setAttribute("onkeyup", "return false"); // event.charCode allows us to set specific keys to use
...
let practice_outcome = {
    ...
    on_load: function () {
        ...
        if (
            ...
            practiceOutcome[practiceIterator] == "win"
        ) {
            if (selection == EasyKey_uCase) {
                outcome.innerHTML =
                    "You won " +
                    currency +
                    practiceEasy[practiceIterator] +
                    " " +
                    points +
                    "!";
                practiceIterator++;
            ...
            feedbackLogic == "You completed the task!" &&
            experimentOutcome[practiceIterator] == "lose"
        ) {
            // elseif block prevents writing bad outcomeLogic i.e. no reward when completed where win was expected
...
let begin_experiment = {
    type: "html-keyboard-response",
    stimulus:
        '<p style="color:white;">That was the practice. The experiment starts now!</p> ' +
        '<p style="color:white;">Get your hands in position and press the spacebar to start. </p>' +
        "<img id='cartoon' src='' width='1000' height='500'>",
    choices: [32],
    on_load: function () {
        "use strict";
...
let experiment_outcome = {
    ...
    on_load: function () {
        ...
        if (
            feedbackLogic == "You completed the task!" &&
            experimentOutcome[experimentIterator] == "win"
        ) {
            if (selection == EasyKey_uCase) {
                outcome.innerHTML =
                    "You won " +
                    currency +
                    easyAmount[experimentIterator] +
                    " " +
                    points +
                    "!";
            ...
            feedbackLogic == "You completed the task!" &&
            experimentOutcome[experimentIterator] == "lose"
        ) {
            // elseif block prevents writing bad outcomeLogic i.e. no reward when completed where win was expected
            outcome.innerHTML = "No " + version + " this round.";
            experimentIterator++;
